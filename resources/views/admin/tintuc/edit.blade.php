...
<div class="box box-warning">
    <!-- /.box-header -->
    <div class="box-body">
        <form action="{{route("admin.tintuc.postedit")}}" method="POST" enctype="multipart/form-data" role="form" style="margin: 30px 5% 30px 5%;">
            <!-- Ten tin tuc -->
            {{csrf_field()}}
            <input style="display:none;" type="text" name="tintuc_id" value="{{$tintuc_id}}">
            ...
            <div class="form-group">
                ...
                <select name="danhmuc_id" class="form-control">
                    ...
                    {{ indanhmuc($objDanhmuccha,$objDanhmuccon,$danhmuc_id) }}
                </select>
            </div>
            <!-- hinh anh -->
            <div class="form-group">
                ...
                <p>
                    <img width="200px" src="/storage/app/files/{{$hinhanh}}">
                </p>
                @if ($errors->has('hinhanhmoi'))
                <div class="alert alert-danger alert-dismissible loi">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                    @foreach ($loi['hinhanhmoi'] as $error)
                        <p><i class="icon fa fa-ban"></i>{{$error}}</p>
                    @endforeach
                </div>
                @endif
                <label>Đổi hình ảnh mới</label>
                <input type="file" name="hinhanhmoi" class="form-control">
                <input style="display:none;" type="text" name="hinhanh" value="{{$hinhanh}}">
            </div>
            <!-- Gioi thieu -->
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                @endif
                <textarea name="chitiet" class="form-control ckeditor" rows="3" placeholder="Nhập chi tiết tin tức...">@if(old('chitiet')){{ old('chitiet') }}@else{{$chitiet}}@endif</textarea>
            </div>
            <!-- active -->
            <div class="form-group">
                ...
                @endif
                <label>
                    <input type="radio" name="active" id="optionsRadios1" value="0" @if( old("active")=="0" || $active == "0") checked @endif >
                    Ẩn
                </label>
                <label style="margin-left:25px;">
                    <input type="radio" name="active" id="optionsRadios2" value="1" @if(old("active")=="1" || $active == "1") checked @endif >
                    Hiện
                </label>
            </div>
            <div class="form-group">
